...
public abstract class AopUtils {
	...
	public static boolean canApply(Pointcut pc, Class<?> targetClass, boolean hasIntroductions) {
		...
		/**
		 * 进行切点表达式的匹配最重要的就是ClassFilter,和MethodMatcher这两个方法的实现
		 * MethodMatcher中有两个matches方法
		 * 一个参数是只有Method对象和targetClass，另一个参数有Method对象和targetClass对象还有一个Method的方法参数
		 * 他们两个的区别是：
		 * 两个参数的matches是用于静态的方法匹配；三个参数的matches是在运行期动态的进行方法匹配的
		 * 先进行classFilter的natches方法校验
		 * 首先这个类要在所匹配的规则下
		 */
		...
		/** 接着获取到targetclass所实现的接口的class对象并加入到集合中 */
	...
	public static boolean canApply(Advisor advisor, Class<?> targetClass, boolean hasIntroductions) {
		if (advisor instanceof IntroductionAdvisor) {
			/**
			 * 如果是IntroductionAdvisor的话，
			 * 则调用IntroductionAdvisor类型的实例进行类的过滤
	...
	public static List<Advisor> findAdvisorsThatCanApply(List<Advisor> candidateAdvisors, Class<?> clazz) {
		...
		/** 定义一个合适的增强器集合对象eligibleAdvisors */
		List<Advisor> eligibleAdvisors = new ArrayList<>();
		/** 循环我们候选的增强器对象 */
		for (Advisor candidate : candidateAdvisors) {
			/** 判断我们的增强器对象是不是实现了IntroductionAdvisor（是否有引介增强，如果没有实现，就不会走这个下面的逻辑)*/
