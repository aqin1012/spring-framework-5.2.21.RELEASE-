...
public class BeanDefinitionParserDelegate {
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		/**
		 * 把当前Bean放入一个状态类（parseState）中，表示正在对他进行解析
		 */
		this.parseState.push(new BeanEntry(beanName));

		/** 解析class属性（有了class其实就可以进行对象的实例化、反射等操作了） */
		...
		try {
			...
			/** 设置description信息（getChildElementValueByTagName()会先判断有没有这个信息） */
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			/**
			 * 仅在spring默认bean的子元素下且为<lookup-method></lookup-method>时有效
			 */
	...
	public void parsePropertyElement(Element ele, BeanDefinition bd) {
		...
		try {
			...
			/**
			 * 解析property的值，把解析结果封装到PropertyValue中，再设置到BeanDefinitionHolder中
			 */
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		 ...
		 * 		1.同时拥有ref和value属性
		 * 		2.同时拥有ref或value属性和子元素
		 * 		3.同时不存在ref和value属性和子元素
		 */

		if (hasRefAttribute) {
			/** ref属性的处理，使用RuntimeBeanReference进行封装 */
